...
export const createWaterQualityStatisticsSchema = z.object({
  ...
  aveDissolvedOxygen: z
    .number()
    .min(0, 'Average dissolved oxygen must be non-negative'),
  aveTurbidity: z.number().min(0, 'Average turbidity must be non-negative'),
  aveOrp: z.number(),
  aveElectricalConductivity: z
    .number()
    .min(0, 'Average electrical conductivity must be non-negative'),
});
...
export const updateWaterQualityStatisticsSchema = z.object({
  ...
  aveTurbidity: z
    .number()
    .min(0, 'Average turbidity must be non-negative')
    .optional(),
...
export type CreateWaterQualityStatisticsInput = z.infer<
  typeof createWaterQualityStatisticsSchema
>;
export type UpdateWaterQualityStatisticsInput = z.infer<
  typeof updateWaterQualityStatisticsSchema
>;
...
export function parseWaterQualityStatisticsKey(
