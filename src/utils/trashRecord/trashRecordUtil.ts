...
export const createTrashRecordSchema = z.object({
  civilianId: z
    .number()
    .int()
    .positive('Civilian ID must be a positive integer'),
  ...
  dateCollected: z
    .string()
    .datetime('Date collected must be a valid datetime')
    .optional(),
  collectorId: z
    .number()
    .int()
    .positive('Collector ID must be a positive integer')
    .optional(),
});
...
export const updateTrashRecordSchema = z.object({
  ...
  segregationScore: z
    .number()
    .min(0, 'Segregation score must be non-negative')
    .optional(),
  recyclingScore: z
    .number()
    .min(0, 'Recycling score must be non-negative')
    .optional(),
  wasteType: wasteTypeEnum.optional(),
  collected: z.boolean().optional(),
  dateDisposed: z
    .string()
    .datetime('Date disposed must be a valid datetime')
    .optional(),
  dateCollected: z
    .string()
    .datetime('Date collected must be a valid datetime')
