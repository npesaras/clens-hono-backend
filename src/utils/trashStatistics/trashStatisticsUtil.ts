...
export const createTrashStatisticsSchema = z.object({
    ...
    leaderboardRank: z.number().int().positive('Leaderboard rank must be a positive integer'),
...
export const updateTrashStatisticsSchema = z.object({
    ...
    leaderboardRank: z.number().int().positive('Leaderboard rank must be a positive integer').optional(),
    totalDisposed: z.number().min(0, 'Total disposed must be non-negative').optional()
});

// Types based on schemas
export type CreateTrashStatisticsInput = z.infer<typeof createTrashStatisticsSchema>;
export type UpdateTrashStatisticsInput = z.infer<typeof updateTrashStatisticsSchema>;
