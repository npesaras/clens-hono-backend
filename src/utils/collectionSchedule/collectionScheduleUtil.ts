...
export const createCollectionScheduleSchema = z.object({
    ...
    collectionTime: z.string().regex(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9]$/, 'Collection time must be in HH:MM:SS format')
});
...
export const updateCollectionScheduleSchema = z.object({
    ...
    collectionDate: z.string().date('Collection date must be a valid date').optional(),
    collectionTime: z.string().regex(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9]$/, 'Collection time must be in HH:MM:SS format').optional()
});

// Types based on schemas
export type CreateCollectionScheduleInput = z.infer<typeof createCollectionScheduleSchema>;
